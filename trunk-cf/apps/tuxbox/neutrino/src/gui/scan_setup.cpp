...
//			strcpy(scanSettings.TP_satname, satList[firstentry].satName);
	...
	if(g_info.delivery_system != DVB_C) {
		...
		scan = new CMenuOptionChooser(LOCALE_SCANTP_SCAN, (int *)&scanSettings.TP_scan, SCANTS_CABLESCAN_OPTIONS, SCANTS_CABLESCAN_OPTION_COUNT, true/*(g_info.delivery_system == DVB_C)*/, &TP_scanNotifier);
	}
