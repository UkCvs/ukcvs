...
$MAIL_TO       = 'tuxbox-cvs@cvs.tuxbox.org';
$CVSWEB        = 'http://cvs.tuxbox.org/tuxbox';
#$CVSWEBROOT    = $1 if $CVSROOT =~ m#/cvsroot/(\S+)#;
...
sub change_summary {
	...
	if (($file =~ /\.(?:pdf|gif|jpg|mpg)$/i) || (-B $file)) {
		...
		if ($rev eq '1.1') {
		    $diff .= "file : $CVSWEB/$rcsfile?rev=$rev&content-type=text/vnd.viewcvs-markup\n";
		    $diff .= "plain: $CVSWEB/$rcsfile?rev=$rev&content-type=text/plain\n\n";
		    open(DIFF, "-|")
		    ...
		    $diff .= "file : $CVSWEB/$rcsfile?rev=$rev&content-type=text/vnd.viewcvs-markup\n";
		    $diff .= "plain: $CVSWEB/$rcsfile?rev=$rev&content-type=text/plain\n";
		    $diff .= "revision graph: $CVSWEB/$rcsfile?graph=$rev\n";
		    $diff .= "diff : $CVSWEB/$rcsfile.diff?r1=$prev_rev&r2=$rev\n\n";
...
sub mail_notification
{
    ...
    open(MAIL, "| mail -s \"$ARGV[0]\" $MAIL_TO");
    print(MAIL join("\n", @text));
    close(MAIL);
}
